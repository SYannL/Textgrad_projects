...
class Optimizer(ABC):
    ...
    def zero_grad(self):
        ...
        for p in self.parameters:
            # Clear the associated contexts and reduction metadata as well.  The
            # optimizer only consumes current-batch feedback, so retaining these
            # objects serves no purpose and grows memory across batches.
            p.reset_gradients()
